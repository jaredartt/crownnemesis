...
- **Match rules: AFK forfeit (2 consecutive turns of zero inputs) and stalemate draw (5 consecutive rounds of 0 total damage), for every mode including the new Battle Royale.** Not built at all yet this session. `royale_players.last_acted_turn` was added as a hook for this but nothing reads it. For 1v1/ranked/tournament, this needs a per-side "turns since last input" counter (probably on `matches.state` itself, or a sibling column) checked from `advance_turn`/`force_timeout`'s existing sweep path, plus a "damage dealt this round" tally reset each round to detect the stalemate condition — `cn_attack`'s `fx.dmg`/`fx.counter` already carry per-exchange damage, so a rolling sum keyed to `turnNumber` is the natural hook.
- **Bots in Battle Royale** — the developer asked, after 0048 shipped, for (a) empty seats in a Vs-Friends royale room to be fillable with bots, and (b) a "4-Player Battle Royale vs Bots" entry point from the existing Vs Bots menu, for solo play against 3 bots. Not started. `bot_step`/`create_bot_match`'s existing logic calls the same `cn_*` functions a human would via a fixed 'host'/'guest' pairing — a royale bot seat needs something calling `submit_royale_*` on a timer/turn-trigger the way `bot_step` does today, for potentially up to 3 seats at once rather than one fixed opponent.
...
- Royale: turn-based status ticking (poison/regen/mist), hidden deployment, 4-way screen rotation, AFK/stalemate once built above.
