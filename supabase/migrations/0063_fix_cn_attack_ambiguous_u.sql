-- =============================================================================
-- 0063 -- fix cn_attack's "column reference \"u\" is ambiguous" crash.
--
-- plpgsql.variable_conflict is 'error' on this project (confirmed live).
-- cn_attack declares `u jsonb;` in its own declare block (the loop variable
-- used everywhere else in the function), and the single-class-team-win
-- achievement check at the bottom of the function does
...
-- which uses `u` as BOTH the FROM-clause alias and (inside u->>'role' etc.)
-- a plain identifier Postgres must resolve against either that alias or the
-- outer plpgsql variable of the same name -- and under variable_conflict =
-- 'error' it refuses to guess, raising 42702 the moment this statement runs.
--
-- Reproduced directly (see chat/session log): a two-line DO block with the
-- identical shape throws the identical error. This statement only runs when
-- v_win_uid is not null and exactly four surviving non-royal units on the
-- winning side share one role -- the "single-class team win" case -- so the
-- bug is narrow but real: that specific match-ending attack currently
-- aborts the whole cn_attack call with a 500 for the player, even though
-- the combat itself had already resolved. Confirmed to break
-- supabase/tests/09_combat.sql, 12_clock.sql and 29_allies_and_flight.sql.
--
-- The fix is the alias rename alone -- `u` -> `uu` in this one statement,
-- nowhere else in the function (every other `for u in select ...` loop in
-- cn_attack has no table alias for `u` to collide with, since jsonb_array_
-- elements() there returns a single unnamed column consumed directly by
-- the loop variable, not referenced by name inside the query text).
--
-- The rest of this CREATE OR REPLACE is byte-identical to the function as
-- fetched live via pg_get_functiondef immediately before writing this file
-- (project ref dnhvfajvfhmqpbwfvyfq), per this project's own "splice, never
-- rewrite from memory" discipline -- diffing this file against that fetch
-- should show exactly one changed region.
-- =============================================================================
...
do $$
declare
  v_out jsonb := '[{"owner":"host","role":"knight","royal":false},{"owner":"host","role":"knight","royal":false}]'::jsonb;
  v_win text := 'host'; v_win_count int; v_win_roles text[];
begin
  -- Same shape as the statement inside cn_attack, run standalone here so
  -- this migration self-verifies rather than only being trusted by eye.
  select count(*), array_agg(distinct uu->>'role') into v_win_count, v_win_roles
    from jsonb_array_elements(v_out) uu
   where uu->>'owner' = v_win and not coalesce((uu->>'royal')::boolean, false);
  if v_win_count <> 2 or v_win_roles <> array['knight'] then
    raise exception 'sanity check failed: count=% roles=%', v_win_count, v_win_roles;
  end if;
  raise notice '0063 sanity check passed: no ambiguous-column error, count=% roles=%', v_win_count, v_win_roles;
end $$;
