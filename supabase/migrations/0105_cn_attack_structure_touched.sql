-- =============================================================================
-- 0105 -- 0104's own structureHits/structureDeaths diff only ever looked at
-- hp, so a destroyed structure's ON_DESTROYED passive that does NOT deal
-- damage or healing -- the live bomb's own row, ADJACENT_UNITS / APPLY_STATUS
-- / BURNING -- never appeared in either field. Board.tsx's newlyAfflicted
-- diff (the unconditional burn/poison/stun flash) was therefore still
-- catching it on the exact same tick the Duel freezes the board -- the same
-- "behind the fight scene" bug 0104 exists to fix, just for a status
-- instead of hp, because 0104 never told the client this unit was touched
-- at all when nothing about its hp moved.
--
-- FIX: the same before/after diff 0104 already runs, widened from "did hp
-- change" to "did this unit's row change at all" (jsonb <>), reported as a
-- new fx field `structureTouched` -- every id the passive laid a hand on,
-- hp or not. `structureHits`/`structureDeaths` are unchanged: still hp-only,
-- for whatever wants exactly a dmg/heal popup. `structureTouched` is a
-- superset client code can diff burn/poison/stun against, the same way
-- Board.tsx's own newlyAfflicted already does for everything else -- see
-- that file's own 0104b comments, shipped in the same commit as this
-- migration.
-- =============================================================================
...
declare
  ...
  -- 0105: every unit the passive touched at all, hp or not -- see this
  -- migration's own header.
  v_structure_touched jsonb := '[]'::jsonb;
begin
  ...
  if v_tree is not null and v_killed_tgt then
    declare
      ...
      v_before_unit jsonb; v_before_hp int; v_after_hp int; v_delta int;
      v_after_ids text[] := '{}';
    begin
      ...
      -- cn_run_effects call.
      --
      -- 0105: v_structure_touched is the WIDER check -- the whole row,
      -- not only hp -- so an APPLY_STATUS with no damage attached (the
      -- live bomb's own ON_DESTROYED) still gets reported. jsonb <>
      -- compares by value, not identity, so a passive that ran and put
      -- the row back exactly as it found it (nothing matched its
      -- target_selector, say) correctly reports no touch at all.
      for u in select * from jsonb_array_elements(v_st->'units') loop
        v_after_ids := v_after_ids || (u->>'id');
        select b into v_before_unit
          from jsonb_array_elements(v_before_su) b where b->>'id' = u->>'id';
        if v_before_unit is null then continue; end if;
        if u <> v_before_unit then
          v_structure_touched := v_structure_touched || to_jsonb(u->>'id');
        end if;
        v_before_hp := (v_before_unit->>'hp')::int;
        v_after_hp := (u->>'hp')::int;
      ...
      -- only to notice. 0105: also counts as touched -- a unit gone is
      -- certainly a unit the passive laid a hand on.
  ...
  v_st := jsonb_set(v_st, '{fx}', jsonb_build_object(
    ...
    'structureTouched', v_structure_touched,
...
do $$
begin
  if (pg_get_functiondef('public.cn_attack(uuid,text,text,text)'::regprocedure) !~ '0105') then
    raise exception '0105 self-check failed: cn_attack missing 0105 marker';
  end if;
end $$;
