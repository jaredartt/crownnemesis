-- =============================================================================
-- 0072 -- HOTFIX: restore SECURITY DEFINER on cn_attack, accidentally
-- dropped by 0067's splice.
--
-- WHY: `select prosecdef from pg_proc where proname = 'cn_attack'` came
-- back false -- every sibling combat/turn function (cn_ability,
-- advance_turn, finish_match, cn_check_achievements) is `true`. Without
-- SECURITY DEFINER, every raw SQL statement inside cn_attack's own body
-- runs under the CALLING PLAYER's RLS context rather than the function
-- owner's, with two confirmed live consequences:
--
--   (a) the single-class-team-win achievement check's
--       `insert into public.player_achievements (...)` was hitting a hard
--       "permission denied for table player_achievements" -- `authenticated`
--       has SELECT/TRUNCATE/REFERENCES/TRIGGER on that table but no INSERT
--       grant at all -- which ABORTED THE ENTIRE cn_attack CALL (a live
--       500) for any match ending with exactly four surviving same-role
--       non-royal units on the winning side. The same crash class 0063
--       exists to fix, reopened by a different statement.
--   (b) `update public.profiles set crit_count/parry_count = ...` for
--       the OTHER player (crediting a parry to the defender, say) was
--       silently updating zero rows under `profiles`' own
--       "own profile updatable" RLS policy (`id = auth.uid()`) -- a
--       silent stat-tracking degradation, not a crash.
--
-- FIX: restore `security definer` in the header. The body below is
-- otherwise byte-identical to 0067's (confirmed via a fresh
-- pg_get_functiondef fetch immediately before writing this migration) --
-- no other logic changes. Ends with a sanity check that raises if this
-- ever regresses again rather than merely selecting a value nobody reads.
-- =============================================================================
...
begin
  if not (select prosecdef from pg_proc
           where proname = 'cn_attack' and pronamespace = 'public'::regnamespace) then
    raise exception 'cn_attack still lacks SECURITY DEFINER after 0072';
  end if;
  raise notice '0072 OK: cn_attack is SECURITY DEFINER again.';
end $$;
