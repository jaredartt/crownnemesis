-- =============================================================================
-- 0067 -- cn_attack gets the same friend/tournament LP gate as claim_win/
-- cn_finish/resign_match (0066): a friend-room or tournament match that
-- ends via cn_attack's own "the crown has fallen" / "no units left" win
-- check now calls finish_match too when the toggle is on, not only when
-- `m.ranked`.
--
-- CAUTION -- HISTORICAL NOTE (see 0072): this splice of cn_attack
-- accidentally dropped `SECURITY DEFINER` from the function header below --
-- it was present on the live function before this migration and is absent
-- here. That regression was live for the rest of this session until 0072
-- restored it with an otherwise byte-identical body. This file is kept
-- exactly as it was actually applied (per this project's "retire never
-- delete, splice forward" convention) rather than silently corrected, so
-- that replaying this repo's migrations in order reproduces the same
-- history -- including the bug -- with 0072 as the real fix on top. Do not
-- copy this file's header as a template for a new function.
--
-- Body is otherwise unchanged from the live function this session started
-- with (already carrying 0063's `uu` rename fix).
-- =============================================================================
...
-- Did it work?
-- ---------------------------------------------------------------------------
select cn_friend_tournament_lp_enabled() is not null as lp_reader_callable;
